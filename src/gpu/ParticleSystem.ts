import particleWgsl from './shaders/particle_update.wgsl?raw';
...
export class ParticleSystem {
  ...
  step(params: {
    ...
    f32[3] = params.dt;
    u32[4] = params.isEmitting ? 1 : 0;
    f32[5] = params.emitRate;
    f32[6] = params.time;
    u32[7] = PARTICLE_COUNT;

    const c = Math.cos(params.rotationAngle);
    const s = Math.sin(params.rotationAngle);
    for (let i = 0; i < 9; i++) f32[10 + i] = 0;
    f32[10 + 0] = c; f32[10 + 2] = s;
    f32[10 + 4] = 1;
    f32[10 + 6] = -s; f32[10 + 8] = c;
    ...
    const groups = Math.ceil(PARTICLE_COUNT / 256);
